...
return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-4 md:p-8">
     ...
     {/* head card */}
        <div className={`bg-[#1e293b] p-8 rounded-2xl border ${isSuperAdmin ? 'border-red-500/50' : 'border-blue-500/50'} shadow-2xl mb-8 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden`}>
          ...
          <div className="relative">
            ...
            {isSuperAdmin && (
                <div className="absolute -top-2 -right-2 bg-yellow-500 text-black p-1.5 rounded-full shadow-lg border-2 border-[#0f172a]" title="Super Admin">
          ...
          <div className="text-center md:text-left z-10">
            ...
            <div className="mt-3 inline-block">
               <span className={`text-xs px-3 py-1 rounded-full font-bold shadow-lg uppercase tracking-widest flex items-center gap-1 ${isSuperAdmin ? 'bg-red-600 text-white' : 'bg-blue-600 text-white'}`}>
                  <ShieldCheck size={12}/> {isSuperAdmin ? "Super Admin" : "Standard Admin"}
               </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <button onClick={() => navigate('/admin/dashboard')} className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 border border-slate-700 transition-all font-semibold">
            ...
            <div className="bg-[#1e293b] rounded-xl border border-slate-700 p-6 md:p-8 min-h-[500px] shadow-xl">
              ...
              {activeTab === 'profile' && (
                <div className="animate-fade-in">
                  <h2 className={`text-2xl font-bold mb-6 border-b border-slate-700 pb-4 flex items-center gap-2 ${isSuperAdmin ? 'text-red-500' : 'text-blue-500'}`}>
                    <User /> {isSuperAdmin ? "Super Admin Details" : "Admin Details"}
                  </h2>
                  ...
                  <form onSubmit={handleUpdateProfile} className="max-w-2xl space-y-8">
                    ...
                    <div className="pt-4 border-t border-slate-700">
                        <button type="submit" className={`px-8 py-3 rounded-lg font-bold transition shadow-lg flex items-center gap-2 text-white ${isSuperAdmin ? 'bg-red-600 hover:bg-red-500' : 'bg-blue-600 hover:bg-blue-500'}`}>
              ...
              {activeTab === 'security' && (
                <div className="animate-fade-in">
                  ...
                  <div className={`bg-slate-900/50 p-6 rounded-xl border ${isSuperAdmin ? 'border-red-500/30' : 'border-blue-500/30'} mb-8`}>
                    <h3 className="text-lg font-bold text-white mb-2">Change Password</h3>
                    <p className="text-slate-400 text-sm mb-6">Update your password to keep your admin account secure.</p>
